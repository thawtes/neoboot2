    ...
    if [ $TARGET = "Flash" ]; then                       
                ...
                if [ -e /.multinfo ]; then                                                                                                                             
                                ...
                                if [ -e $NEOBOOTMOUNT/ImagesUpload/.kernel/flash-kernel-$BOXNAME.bin ] ; then                                                                                                                                                                                                                       
                                    if [ -d /proc/stb ] ; then
                      	    	            dd if=$NEOBOOTMOUNT/ImagesUpload/.kernel/flash-kernel-$BOXNAME.bin of=/dev/mmcblk0p3
                                    fi
                                    echo "Boot - Flash. "                                                                                                                                                                                        
                                    ...
                                    if [ -e $NEOBOOTMOUNT/ImagesUpload/.kernel/flash-kernel-$BOXNAME.bin ] ; then
                                        ...
                                        if [ -d /proc/stb ] ; then
                                                    dd if=$NEOBOOTMOUNT/ImagesUpload/.kernel/flash-kernel-$BOXNAME.bin conv=noerror conv=sync of=/dev/mmcblk0p3
                                        fi                                                                                                                                              
